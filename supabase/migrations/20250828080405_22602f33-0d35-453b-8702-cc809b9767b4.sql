...
-- Create function to generate internal names for existing records
...
DECLARE
  ...
  clean_title TEXT;
BEGIN
  -- Clean title: remove special chars, take first 20 chars, uppercase
  ...
  clean_title := LEFT(clean_title, 20);
  ...
  -- Ensure not too long
  RETURN LEFT(base_name, 50);
END;
$$ LANGUAGE plpgsql;

-- Update existing content_ideas with generated internal names
UPDATE public.content_ideas 
SET internal_name = generate_internal_name(title, content_type, target_audience, NULL, NULL, created_at)
WHERE internal_name IS NULL;

-- Update existing content_briefs with generated internal names  
UPDATE public.content_briefs 
SET internal_name = generate_internal_name(title, brief_type, target_audience, NULL, NULL, created_at)
WHERE internal_name IS NULL;

-- Update existing content_items with generated internal names
UPDATE public.content_items 
SET internal_name = generate_internal_name(title, content_type, NULL, NULL, NULL, created_at)
WHERE internal_name IS NULL;

-- Update existing content_derivatives with generated internal names
UPDATE public.content_derivatives 
SET internal_name = generate_internal_name(title, content_type, NULL, derivative_type, category, created_at)
WHERE internal_name IS NULL;

-- Update existing general_content_items with generated internal names
