...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'trigger') {
      ...
      // For content creation submissions, we'll simulate processing and create a callback
      if (submission.knowledge_base === 'content_creation') {
        console.log('Processing content creation submission');
        ...
        // For now, simulate successful processing by calling our own callback
        // In a real scenario, this would be done by your N8N workflow
        const callbackPayload = {
          submission_id: submissionId,
          status: 'completed',
          content_data: {
            title: `Generated Content: ${submission.original_filename}`,
            content: 'This is a sample generated content item. In a real implementation, this would be generated by your AI processing pipeline.',
            summary: 'A brief summary of the generated content.',
        ...
        console.log('Simulating callback with payload:', callbackPayload);
        
        // Trigger the callback processing
        setTimeout(async () => {
          try {
            const callbackUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/process-content?action=callback`;
        ...
        return new Response(
          JSON.stringify({ 
            success: true, 
            message: 'Content creation processing started',
            submission_id: submissionId 
          }),
      ...
      if (status === 'completed' && submission.knowledge_base === 'content_creation' && content_data) {
        ...
        try {
          ...
          // Since we don't have a direct link, we'll try to find the most recent brief for this user
          ...
          if (recentBriefs && recentBriefs.length > 0) {
            // Find a brief that hasn't been used yet
