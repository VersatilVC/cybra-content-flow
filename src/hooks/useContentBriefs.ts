...
export function useContentBriefs(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      let q = supabase
        .from('content_briefs')
        .select('id,user_id,source_id,source_type,title,description,brief_type,target_audience,status,created_at,updated_at,content', { count: 'exact' })
    ...
    if (options?.page && options?.pageSize) {
      ...
      queryClient.prefetchQuery({
        ...
        queryFn: async () => {
          let q = supabase
            .from('content_briefs')
            .select('id,user_id,source_id,source_type,title,description,brief_type,target_audience,status,created_at,updated_at,content')
            .order('created_at', { ascending: false });
