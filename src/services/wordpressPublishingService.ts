...
export { WordPressPublishPayload } from './wordpress/types';
...
export async function publishToWordPress(contentItem: ContentItem, userId: string): Promise<void> {
  ...
  // Build the WordPress publish payload
  const payload = buildWordPressPayload(contentItem, userId, derivatives || undefined);
